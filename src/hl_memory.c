...
int hl_memory_destroy(struct memory_ctx *ctx) {
    ...
    (void) pthread_mutex_destroy(&hl_ctx->mem_table_device_lock);
    kh_destroy(uint64_t, hl_ctx->mem_table_device);
    (void) hlthunk_close(hl_ctx->device_fd);
...
int hl_memory_free_buffer(struct memory_ctx *ctx, int dmabuf_fd, void *addr, uint64_t size) {
    ...
    uint64_t device_handle = INVALID_FD;
    khint_t k;
    ...
    device_handle = kh_val(hl_ctx->mem_table_device, k);
    rc = hlthunk_device_memory_free(hl_ctx->device_fd, device_handle);
